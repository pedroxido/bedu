...
class CognitoExpress {
    constructor(config) {
        ...
        if (configurationIsCorrect(config)) {
            ...
            this.host = `cognito-idp.${config.region}.amazonaws.com`;
            this.path = `/${this.userPoolId}/.well-known/jwks.json`;
                ...
                return new Promise((resolve, reject) => {
                    jwtVerify(params, (err, result) => {
                        if (err) {
                            ...
                            this.currentEmail = decodedJwt.payload.email;
                            resolve(result);
                        }
                    });
                });
